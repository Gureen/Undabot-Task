...
const surveyFormAnswers = require(path.join(__dirname, './store/SurveyFormData.json'));
app.use(cors({ origin: '*' }));
app.get('/api/v1/survey', (request, response) => {
    fs.readFile(path.join(__dirname, './store/SurveyFormData.json'), 'utf-8', (error, data) => {
        if (error) {
...
app.post('/api/v1/survey/:id/answers', (request, response) => {
    ...
    //validation
    surveyFormAnswers === null || surveyFormAnswers === void 0 ? void 0 : surveyFormAnswers.attributes.questions.forEach((q) => {
        if (q.required) {
            const findQuestionAnswer = answers.find((ans) => ans.questionId === q.questionId);
    ...
    fs.readFile(path.join(__dirname, './store/MovieReviews.json'), 'utf-8', (error, data) => {
        if (error) {
            return response.status(500).send({
                errors: [
                    {
        ...
        fs.writeFile(path.join(__dirname, './store/MovieReviews.json'), JSON.stringify(request.body), () => {
